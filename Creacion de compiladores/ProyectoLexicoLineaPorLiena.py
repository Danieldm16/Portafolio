...
matrizdeestados = [[203, 2, 2, 4, 4, 6, 6, 8, 8, 105, 10, 107, 117, 119, 200, 200, 16, 17, 200, 200, 200, 21, 22, 200],
        [140, 200, 100, 201, 101, 202, 103, 202, 102, 105, 104, 107, 117, 119, 200, 200, 16, 17, 200, 200, 200, 21, 22, 200],
...
def obtenercolumna(caracter, estadoanterior):#Recibe un caracter
...
def analizarcodigofuente(inftxt, matrizdeestados):#Función que analiza el codigo fuente, recibe la información del codigo fuente y la matriz de estados
    ...
    for linea in inftxt:#El for itera sobre todas las lineas de el codigo fuente
        ...
        while contador < len(linea):#Este while itera sobre cada caracter en cada linea
            ...
            columna = obtenercolumna(caracter, estadoanterior)#se ovtiene la columna a la que pertenece el caracter
            ...
            if estado >= 100:
                ...
                if estado == 101:
                    ...
                    estado = 0
                    cadenaencontrada = loqueseencuentra.strip()
                    print(f"Se encontro el numero decimal: {cadenaencontrada[:-1]}")
                    loqueseencuentra = ""
                if estado == 102:
                    contador-=1
                    estado = 0
                    cadenaencontrada = loqueseencuentra.strip()
                    print(f"Se encontro el numero natural con notación cientifica: {cadenaencontrada[:-1]}")
                    loqueseencuentra = ""
                if estado == 103:
                    contador-=1
                    estado = 0
                    cadenaencontrada = loqueseencuentra.strip()
                    print(f"Se encontro el numero decimal con notación cientifica: {cadenaencontrada[:-1]}")
                    loqueseencuentra = ""
                ...
                if estado == 128:
                    estado = 0
                    cadenaencontrada = loqueseencuentra.strip()
                    print(f"- Se encontro un salto de linea -")
                    loqueseencuentra = ""
                if estado == 129:
                    estado = 0
                    cadenaencontrada = loqueseencuentra.strip()
                ...
                if estado == 140:
                    estado = 0
                    cadenaencontrada = loqueseencuentra.strip()
                    print(f"- Se encontro un espacio -")
                    loqueseencuentra = ""
                if estado == 141:
                    estado = 0
                    cadenaencontrada = loqueseencuentra.strip()
                    print(f"Se encontro un tabulador: {cadenaencontrada}")
                    loqueseencuentra = ""
                if estado == 200:
                    contador-=1
                    estado = 0
                    print(f'ERROR: El siguiente caracter o conjunto de caracteres no se reconoce: {loqueseencuentra}')
                    loqueseencuentra = ""
                if estado == 201:
                    contador-=1
                    estado = 0
                    print(f'ERROR: Un numero decimal no puede quedar asi: {loqueseencuentra}, debe continuar con otro digito')
                    loqueseencuentra = ""
                if estado == 202:
                    contador-=1
                    estado = 0
                    print(f'ERROR: Un numero natural o decimal con notacion cientifica no puede quedar asi: {loqueseencuentra[1:-1]}, debe seguir con un otro digito')
                    loqueseencuentra = ""
        ...
        if estado == 22:#si el estado anterior es 22 (este es el estado de los comentarios de varias lineas)
            #y se cambia de linea entonces seguira en el estado 22, para que continue escribiendo ese comentario, si 
            # no esta en ese estado se regresa al estado 0 después de acabar cada linea
            estado = 22
        else:
            estado = 0
