...
public class DijkstraAlgorithm 
{
    ...
    {
        Scanner s = new Scanner( System.in );//para lectura de datos
        
        String temp = s.nextLine();
        
        String temp2[] = temp.split(" ");
        
        int [] nodoscallescasaoficina = new int[temp2.length];
        
        for (int i = 0; i < nodoscallescasaoficina.length; i++) 
        {
            nodoscallescasaoficina[i] = Integer.parseInt(temp2[i]);
        }
        
        Dijkstra dijkstraAlgorithm = new Dijkstra(nodoscallescasaoficina[0]);
        
        for( int i = 0 ; i < nodoscallescasaoficina[1] ; ++i )
        {
            String temp3 = s.nextLine();
        
            String temp4[] = temp3.split(" ");
        
            int [] calles = new int[temp4.length];
            
            for (int j = 0; j < calles.length; j++)
            {
                calles[j] = Integer.parseInt(temp4[j]);
            }
            
            //Aquí se usa dos veces ya que nuestro peso es igual de x a y que de y a x, si solo se dejará una linea serían aristas con dirección
            //haciendo que de x a y fuera un peso distinto a y a x.
            dijkstraAlgorithm.addEdge(calles[0], calles[1], calles[2], true);
            dijkstraAlgorithm.addEdge(calles[1], calles[0], calles[2], true);
        }
        
        dijkstraAlgorithm.dijkstra(nodoscallescasaoficina[2]);
        
        System.out.println("");//se pasa una linea para que el camino se imprima solo
        
        dijkstraAlgorithm.printShortestPath(nodoscallescasaoficina[3]);
    }
}
...
class Dijkstra{
    ...
    void dijkstra( int inicial )
    {
        ...
        //Estás lineas nos dan la distancia más corta (el valor de la distancia, no el camino), de nuestro origen a cada uno de los otros nodos
        //pero está comentarizada por que en está práctica no nos la solicitan
        //----------------------------------------------------------------------------------------------------------------------------------------------
        //System.out.println("");//se pasa la linea por estetica
